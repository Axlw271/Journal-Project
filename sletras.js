    ...
    const dictionary = {
        ...
        function searchWord(word) {
          ...
          imageBox.innerHTML = ''; // Limpiar imágenes anteriores   
          for (let i = 0; i < word.length; i++) {
            const letter = word[i];
            console.log(word[i])
            if (dictionary.hasOwnProperty(letter)) {
              const img = document.createElement('img');
              img.src = dictionary[letter];
              img.style.maxWidth=window.innerWidth/(word.length+1)+"px";
    ...
    for(i=0;i<n;i++){
        document.writeln("<tr>")
        for(j=0;j<n;j++){
            document.writeln("<td style='width :"+sz+"px; height: "+sz+"px;' id='"+i+","+j+"' onclick=high("+i+","+j+")>"+sopa[i][j]+"</td>")
        }
        document.writeln("</tr>")
    }
    document.writeln("</table>")
    ...
    function registro(){
        const data = new URLSearchParams("act_id=slet & resultado=10");
        fetch('../resultado.php', {
        method: 'POST',
        body: data
        })
